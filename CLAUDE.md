...
- Reviewers are subagents in `.claude/agents/`: invoke `code-reviewer`,
  `design-reviewer`, `research-reviewer` or `context-reviewer` fresh every
  round with the full brief (docs/practices/review.md; `context-reviewer`
  states its own). Subagent definitions load when a session starts.
