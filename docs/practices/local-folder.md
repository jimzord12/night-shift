...
```text
<project>/.local/                  git-ignored; also in .git/info/exclude
  preferences/                     the owner's personal profile: read every file, every session
    owner-profile.md               (templates/local/owner-profile.md)
...
Add both lines when adopting it:

```sh
echo "/.local/" >> .gitignore
echo "/.local/" >> .git/info/exclude     # still ignored if .gitignore is ever edited carelessly
```

## Preferences

- **Read every Markdown file in `.local/preferences/`** before the first
  reply of a session, together with the shared owner file (`docs/owner.md`,
  [owner.md](owner.md)). Missing folder: use the owner file alone.
...
- Never copy its contents into a commit, a card, a report, a log or a
...
talks to any owner of this project**; the profile says **who this owner is
...
- When the owner accepts it, promote it: a decision entry, a doc, or tasks
  on the board. Then delete the draft or mark its README "promoted to …".
...
needs is attached to the card or committed next to the work instead
